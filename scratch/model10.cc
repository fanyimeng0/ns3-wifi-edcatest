...
void
Tx4callback(Ptr<const Packet>packet,double energy)
{
  Ptr<Packet> m_currentPacket;
  WifiMacHeader hdr;
  m_currentPacket = packet->Copy();
  m_currentPacket->RemoveHeader (hdr);
  if(hdr.IsData()){
    m_phy4TxBegin=m_phy4TxBegin+packet->GetSize();
    /*std::cout << "PHY TX :" << m_phyTxBegin << " " << std::endl;
    std::cout << "GetAddr1 :" << hdr.GetAddr1() << " " << std::endl;
    std::cout << "GetAddr2 :" << hdr.GetAddr2() << " " << std::endl;
    std::cout << "type :" << hdr.GetTypeString() << std::endl;
...
showresult()
{
  ...
  double total=m_phy1TxBegin+m_phy2TxBegin+m_phy3TxBegin+m_phy4TxBegin;
  std::cout << "Radio1: \t" << m_phy1TxBegin/total <<  std::endl;
  std::cout << "Radio2: \t" << m_phy2TxBegin/total <<  std::endl;
  std::cout << "Radio3: \t" << m_phy3TxBegin/total <<  std::endl;
  std::cout << "Radio4: \t" << m_phy4TxBegin/total <<  std::endl;
  ...
  std::cout << "Throughput4: \t" << m_phy4TxBegin*8/1000000 <<  std::endl;
...
{ 
  ...
  Config::Set ("/NodeList/1/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/BK_Txop/MinCw",  UintegerValue (3));
  Config::Set ("/NodeList/1/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/BK_Txop/MaxCw",  UintegerValue (1023));

  Config::Set ("/NodeList/2/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/BK_Txop/MinCw",  UintegerValue (63));
  Config::Set ("/NodeList/2/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/BK_Txop/MaxCw",  UintegerValue (1023));

  Config::Set ("/NodeList/3/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/BK_Txop/MinCw",  UintegerValue (127));
  Config::Set ("/NodeList/3/DeviceList/*/$ns3::WifiNetDevice/Mac/$ns3::RegularWifiMac/BK_Txop/MaxCw",  UintegerValue (1023));
  ...
  Config::ConnectWithoutContext("/NodeList/3/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyTxBegin",MakeCallback(&Tx4callback));
