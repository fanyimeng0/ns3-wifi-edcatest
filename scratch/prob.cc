...
void
Tx1callback(Ptr<const Packet>packet)
{
...
void
Tx2callback(Ptr<const Packet>packet)
{
  Ptr<Packet> m_currentPacket;
  WifiMacHeader hdr;
  m_currentPacket = packet->Copy();
...
showresult()
{
  ...
  std::cout << "Radio1: \t" << m_phy1TxBegin/(m_phy1TxBegin+m_phy2TxBegin) <<  std::endl;
  std::cout << "Throughput1: \t" << m_phy1TxBegin*1472*8/1000000 <<  std::endl;
  std::cout << "Throughput2: \t" << m_phy2TxBegin*1472*8/1000000 <<  std::endl;
...
{ 
  ...
  Config::ConnectWithoutContext("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Mac/MacTx",MakeCallback(&Tx1callback));
  Config::ConnectWithoutContext("/NodeList/1/DeviceList/*/$ns3::WifiNetDevice/Mac/MacTx",MakeCallback(&Tx2callback));
  if(1==0){
