...
PoissonApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PoissonApplication")
    ...
    .AddAttribute ("Interval", "The time between packets sent in on state",
                   StringValue ("ns3::ConstantRandomVariable[Constant=1.0]"),
                   MakePointerAccessor (&PoissonApplication::m_interval),
                   MakePointerChecker <RandomVariableStream>())
    ...
    .AddAttribute ("Tagvalue", 
                   "The total number of bytes to send. Once these bytes are sent, "
                   "no packet is sent again, even in on state. The value zero means "
                   "that there is no limit.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&PoissonApplication::m_tagvalue),
                   MakeUintegerChecker<uint64_t> ())
...
void 
PoissonApplication::SetTagvalue(uint64_t  tagvalue)
{
...
void PoissonApplication::CancelEvents ()
{
  ...
  if (m_sendEvent.IsRunning () && m_cbrRateFailSafe == m_cbrRate )
    { // Cancel the pending send packet event
      ...
      Time delta (m_lastStopTime - m_lastStartTime);
...
void PoissonApplication::ScheduleNextTx ()
{
  ...
  if (m_maxBytes == 0 || m_totBytes < m_maxBytes)
    ...
    m_poisson=m_interval->GetValue();
    Time nextTime (Seconds (bits /
                              static_cast<double>(m_cbrRate.GetBitRate ())+m_poisson)); // Time till next packet
      NS_LOG_LOGIC ("nextTime = " << nextTime.As (Time::S));
      m_sendEvent = Simulator::Schedule (nextTime,
                                         &PoissonApplication::SendPacket, this);
...
{  // Schedules the event to start sending data (switch to the "On" state)
  ...
  m_startStopEvent = Simulator::Schedule (offInterval, &PoissonApplication::StartSending, this);
}
...
{  // Schedules the event to stop sending data (switch to "Off" state)
  ...
  m_startStopEvent = Simulator::Schedule (onInterval, &PoissonApplication::StopSending, this);
}
